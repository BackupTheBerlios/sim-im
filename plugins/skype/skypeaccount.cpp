...
class SkypeAccountPrivate {
	public:
		...
		SIM::MetaContact notInListUsers;
		///Constructor
...
SkypeAccount::SkypeAccount(SkypeProtocol *protocol) : SIM::Account(protocol, "Skype", (char *)0) {
	...
	SkypeContact *_myself = new SkypeContact(this, "Skype", SIM::ContactList::self()->myself(), false);
	setMyself(_myself);
...
bool SkypeAccount::createContact(const QString &contactID, SIM::MetaContact *parentContact) {
	kdDebug(14311) << k_funcinfo << endl;//some debug info
...
void SkypeAccount::setOnlineStatus(const SIM::OnlineStatus &status, const QString &) {
	kdDebug(14311) << k_funcinfo << endl;//some debug info
...
void SkypeAccount::connect(const SIM::OnlineStatus &Status) {
	kdDebug(14311) << k_funcinfo << endl;//some debug info
...
void SkypeAccount::sendMessage(SIM::Message &message, const QString &chat) {
	kdDebug(14311) << k_funcinfo << endl;//some debug info
...
QString SkypeAccount::getUserLabel(const QString &userId) {
	...
	SIM::Contact *cont = contact(userId);
...
void SkypeAccount::receiveMultiIm(const QString &chatId, const QString &body, const QString &messageId, const QString &user) {
	...
	SIM::Message mes(getContact(user), myself(), body, SIM::Message::Inbound);
...
QPtrList<SIM::Contact> *SkypeAccount::constructContactList(const QStringList &users) {
	QPtrList<SIM::Contact> *list= new QPtrList<SIM::Contact> ();
