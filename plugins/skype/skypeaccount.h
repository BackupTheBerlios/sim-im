 ...
 */
class SkypeAccount : public SIM::Account
{
Q_OBJECT
	private:
		...
		QPtrList<SIM::Contact> *constructContactList(const QStringList &users);
	private slots:
		/**
	...
	protected:
		 ...
		 */
		virtual bool createContact(const QString &contactID, SIM::MetaContact *parentContact);
	public:
		/**
		 * Constructor.
		...
		bool canAlterAuth();
		/**
		 * How shoul SIM authorize it self? (empty means as SIM)
		 */
		QString author;
		/**
	...
	public slots:
		 ...
		 */
		virtual void setOnlineStatus(const SIM::OnlineStatus &status, const QString &reason);
		/**
		 ...
		 */
		virtual void connect(const SIM::OnlineStatus &status);
		/**
		 ...
		 */
		void sendMessage(SIM::Message &message, const QString &chat);
		/**
		...
		/**
		 * Enables reading messages by SIM. If it is on, all messages showed in SIM will be marked as read, if disable, you will have to read them in Skype/something else.
		...
		/**
		 * Set if there should be scan for unread messages when SIM connects to Skype.
