 ...
 */
class SkypeChatSession : public SIM::ChatSession
{
	Q_OBJECT
	private:
	...
	public:
		 ...
		 */
		SkypeChatSession(SkypeAccount *account, const QString &session, const SIM::ContactPtrList &contacts);
		///Destructor
		~SkypeChatSession();
		/**
	...
	public slots:
		 ...
		 * @param recv List of receivers. If there are more than one, replaced by an dummy contact of that chat, because it does crash SIM otherwise
		 * @param body Text to show
		 */
		void sentMessage(const QPtrList<SIM::Contact> *recv, const QString &body);
	signals:
		/**
