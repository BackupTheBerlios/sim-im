 ...
 */
class SkypeContact : public SIM::Contact
{
	Q_OBJECT
	private:
	...
	public:
		 ...
		 */
		SkypeContact(SkypeAccount *account, const QString &id, SIM::MetaContact *parent, bool user = true);
		/**
		 * Destructor.
		 */
		~SkypeContact();
		 ...
		 */
		virtual SIM::ChatSession *manager(SIM::Contact::CanCreateFlags flags);
		/**
		 * Save this contact (resp. set what should be saved and it will be written automatically by SIM)
		 */
		...
		bool hasChat() const;
		///Tell SIM which actions to show in the contact pop-up menu
